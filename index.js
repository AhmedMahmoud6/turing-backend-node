...
if (!APPSCRIPT_URL) {
  console.warn('APPSCRIPT_URL not set; endpoint will return 500 until configured');
}
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    if (!APPSCRIPT_URL) {
      console.error('APPSCRIPT_URL not configured; cannot forward registration');
      // update Firestore doc with error if created
      if (docRef) await docRef.update({ email_error: 'APPSCRIPT_URL not set', email_requested: false });
      return res.status(500).send('server misconfiguration: APPSCRIPT_URL not set');
    }
    ...
    console.log('Forwarding registration to Apps Script (JSON)', { workshopId, email, name });
    let resp;
    try {
      ...
      if (docRef) await docRef.update({ email_sent: false, email_error: String(err), email_requested: false });
      throw err;
    }
    ...
    if (!resp.ok || (parsed && parsed.error)) {
      ...
      console.error('Apps Script returned non-OK or error field', resp.status, errMsg);
      if (docRef) await docRef.update({ email_sent: false, email_response: text, email_requested: false });
    ...
    if (docRef) {
      try {
        await docRef.update({ email_sent: true, email_response: text, emailedAt: admin.firestore.FieldValue.serverTimestamp(), email_requested: false });
      } catch (err) {
